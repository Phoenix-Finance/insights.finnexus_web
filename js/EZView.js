...
$.fn.EZView = function () {
    ...
    self.createContainer = function () {
        if (!$('#EZView').length) {
            var tools = '<div class="tools-container">' +
    ...
    self.setObjectTemplate = function (object) {
        $EZView.find('.object').append(object[0]).find('.download, img').click(function (e) {
            e.stopPropagation();
        });
    ...
    self.setStyles = function () {
        $EZView.css({
            ...
            'padding': '7px',
        }).end().find('.close>img').css({
            'top': '0',
            'right': '0',
            'position': 'fixed',
        }).end().find('.back>img').css({
            'top': $(window).height() * 0.5,
            'left': '0',
            'position': 'fixed',
        }).end().find('.next>img').css({
            'top': $(window).height() * 0.5,
            'right': '0',
            'position': 'fixed',
        }).end().find('.tools-container').css({
            ...
            'padding-top': '0px',
            'z-index': '100000'
        }).end().find('.tools-container>a').css({
            'margin': '10px',
        }).end().find('.tools').css({
            'top': '0',
            'left': '0',
            'position': 'fixed',
        }).end().find('.EZViewHighlight>img').css({
            'padding': '10px',
            'height': 'auto',
            'width': 'auto',
            'z-index': '100000',
            'cursor': 'pointer',
        }).end().find('.EZViewHighlight>img').hover(function () {
            $(this).css({
                'border-radius': '100%',
                'background-color': 'rgba(255,255,255,0.5)',
                'z-index': '100000'
    ...
    self.addEvents = function () {
        $EZView.click(function (e) {
            ...
            e.stopPropagation();
            self.close();
        }).end().find('.download, img, .tools').click(function (e) {
            e.stopPropagation();
        }).end().find('.back>img').click(function (e) {
            e.stopPropagation();
            e.preventDefault();
            self.back();
        }).end().find('.next>img').click(function (e) {
            e.stopPropagation();
            e.preventDefault();
            self.next();
        }).end().find('.zoomin, .zoomout, .close').click(function (e) {
            e.stopPropagation();
            e.preventDefault();
        }).end().find('.zoomin>img').click(function (e) {
            e.stopPropagation();
            e.preventDefault();
            self.zoom(true);
        }).end().find('.zoomout>img').click(function (e) {
            e.stopPropagation();
            e.preventDefault();
            self.zoom();
        }).end();
